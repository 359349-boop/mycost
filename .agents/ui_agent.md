...
- 统计页收支分析（Trend）：
  - 顶部卡片：大圆角卡片（`.cornerRadius(16)`），沿用 `secondarySystemBackground`。
  - 趋势图：收入/支出双柱，收入绿、支出红；柱体整体左移 `offset(x: -16)`。
  - 滑动方式：横向连续滑动（非翻页），默认展示当前月向左 6 个月，仅可查看历史月份。
  - 轴线：0 轴实线；中值/最大值虚线为灰色（`systemGray3`）。
  - k 值：显示当前可视 6 个月的最大值与中值，文本固定在图表最左侧。
  - 月份标注：显示 `MM`，位置位于收入/支出双柱之间。

## 任务执行
- 负责 `Views/` 下的所有 SwiftUI 文件编写。
- 确保所有页面在浅色和深色模式下通过对比度检查。
- 组件需具备良好可访问性（动态字体、对比度）。
- 当系统键盘弹出（搜索或输入）时，悬浮“加号”按钮需要隐藏，不得上移到键盘上方。
...
- 分类选择：独立卡片展示（不要与计算器面板同层/同区）。卡片内部可纵向滚动，隐藏右侧滚动条。
...
- 分类网格：每行 5 个图标；默认选中第一个图标。
  - 未选中显示中性风格，选中显示分类色。末尾提供“设置”入口（图标 `gearshape`），跳转到分类管理页，颜色与其它图标一致。
