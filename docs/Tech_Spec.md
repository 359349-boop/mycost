...
- 收支分析（Trend）：
  - 图表：使用 `BarMark` 绘制收入/支出双柱趋势（月份页按月、年份页按年）。
  - 滑动：启用 `.chartScrollableAxes(.horizontal)`；月份页默认可视 6 个月，年份页默认可视 6 年。
  - 数据范围：仅包含历史时间桶（月份页不展示未来月，年份页不展示未来年）。
  - 动态尺度：y 轴最大值/中值基于当前可视窗口计算。
  - 可视窗口判定：以当前滚动右边界时间桶为锚点，向左回推固定窗口长度（6 个月或 6 年）。
  - 点击交互：点击柱体后吸附到最近时间桶并更新选中期间（`selectedMonth` / `selectedYear`），驱动下方 Donut 与分类列表切换。
